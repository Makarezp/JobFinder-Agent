...
**IMPLEMENTED — LIVE VERIFICATION PENDING (2026-08-08).** Changes live in `.agent/skills/agent-tabs/` (agentctl.py + 3 test files), uncommitted. Unit and tmux-backed checks are complete. The required opt-in live-worker test did not pass, so T0 remains pending and neither this status nor the T0 entries in the README and sprint register were changed to Done.
...
- **Likely launch blocker:** During the attached manual spawn, the human observed a Claude Code confirmation dialog for bypass-permissions mode and had to approve it. The manual `spawn` returned only after `76.12s`; the headless E2E uses the same `--permission-mode bypassPermissions` launch option with `--viewer none`, so no one can approve such a dialog. This is consistent with, but not independently proven by, the failed E2E because its diagnostic capture was untrusted. It must be eliminated or programmatically pre-accepted before the required test can pass.
...
**Disposition:** The manual checks pass, but the required live-worker regression test fails despite available Claude spend. T0 is not done.
