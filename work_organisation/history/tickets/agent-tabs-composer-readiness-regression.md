...
- **Claude Code:** `2.1.226 (Claude Code)`. Spend is available: `claude -p 'Reply exactly: READY'` returned `READY`; there was no spend-limit refusal.
- **Exact required command:** `AGENT_TABS_E2E=1 AGENTCTL_PYTHON="$PWD/.venv/bin/python" .agent/skills/agent-tabs/test.sh`
- **Result:** ruff passed, formatting check passed, mypy passed (`11 source files`), and pytest reported `1 failed, 198 passed in 116.54s (0:01:56)`. The required `test_composer_gate_and_send_against_a_live_worker` failed before its send assertions: `agentctl.SpawnError: agent 't0live' never reported SessionStart within 90s.` Its exact diagnostic was `Last screen:` followed by `screen capture unavailable (unattached pane)`.
- **Launch diagnosis:** The initial 90-second timeout cannot be attributed to the observed bypass-permissions UI. The user-level Claude configuration already contains `skipDangerousModePermissionPrompt: true`. A visible diagnostic worker subsequently emitted `spawned`, `turn_start`, `reply`, and `turn_end`; an immediate second run with `AGENT_TABS_VIEWER=none` completed `spawn` in `2.23s`. The original screen capture was explicitly untrusted, so the dialog remains an unproven environmental condition. No repository code may pre-accept it.
- **Manual attached-worker delivery:** Spawned `t0manual` using `AGENT_TABS_RUNTIME=/private/tmp/agent-tabs-t0-manual-20260808 AGENT_TABS_RUN=t0manual20260808 AGENT_TABS_VIEWER=iterm-tab "$PWD/.venv/bin/python" .agent/skills/agent-tabs/agentctl.py spawn t0manual --role .agent/skills/agent-tabs/examples/reviewer-role.md --task 'Open your inbox, then reply exactly MANUAL_READY via agentctl reply and wait.' --provider claude --model haiku --permission-mode bypassPermissions --spawn-timeout 90 --bootstrap-timeout 60 --viewer iterm-tab`. It became idle and replied `MANUAL_READY`. `agentctl send t0manual 'Read the new inbox instruction. Reply exactly DOORBELL_RECEIVED via agentctl reply, then wait.'` exited `0`; the screen showed `[orchestrator] new instruction:` for inbox `0002.md`; the worker read that file, ran `agentctl reply`, and reported `DOORBELL_RECEIVED`.
- **Manual dirty-composer guard:** Typed the unsent text `human unsent T0 dirty composer guard` into the idle worker's composer. `agentctl send t0manual --queue 'QUEUED_MESSAGE_MUST_REMAIN_UNREAD'` exited `3` and printed `queued .../inbox/0003.md human_typing`. `0003.md` remained in the inbox with that payload, and a subsequent screen capture still showed the exact human text unchanged. The manual run was then closed with `agentctl close-run --timeout 10`.

**Corrected live-test run — 2026-08-08:** The initial manual checks remain valid. The first live attempt exposed two test failures: its Composer test treated a transient screen doorbell as a durable assertion despite `bus.jsonl`/outbox being the protocol's machine channels, and the pre-existing handshake test gave a hidden Sonnet worker an unbounded review task under `acceptEdits`. The tests now wait for exact durable outbox replies: `OK` after the T0 doorbell and `HANDSHAKE_READY` after a bounded Haiku handshake using the explicit `bypassPermissions` mode. The required command now passes: ruff and formatting clean, mypy strict clean (`11 source files`), `199 passed in 81.31s (0:01:21)`. No production `agentctl` behavior changed in this correction.
