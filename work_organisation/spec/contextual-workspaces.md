...
*   [ ] **Backend Specialization & Onboarding Deprecation:** Split the current unified generic agent graph into smaller, focused graphs (e.g., `profile_graph.py` and `discovery_graph.py`).
    ...
    *   **Shared Memory Tools:** Both agents MUST retain access to profile-updating tools (e.g., `update_my_profile`). The Discovery Agent specifically needs this to record new exclusions or preferences learned during the job search conversation (e.g., "I hate Java, don't show me that again").
...
*   [ ] Messages sent from the Discovery tab successfully trigger job searches and can update profile preferences/exclusions as needed to maintain the learning loop.
