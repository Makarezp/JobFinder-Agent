...
describe("chat API clients", () => {
    ...
    describe("fetchHistoryRequest", () => {
        ...
        it("throws an error on failure", async () => {
            ...
            await expect(fetchHistoryRequest()).rejects.toThrow("HTTP error! status: 500");
        });
    });

    describe("sendMessageRequest", () => {
        it("successfully sends a message", async () => {
            const mockResponse = { user_message: "test", ai_message: "reply", jobs: [] };
            vi.mocked(fetch).mockResolvedValueOnce({
                ok: true,
        ...
        it("throws an error on failure", async () => {
            ...
            await expect(sendMessageRequest("fail")).rejects.toThrow("HTTP error! status: 400");
        });
    });

    describe("uploadCVRequest", () => {
        it("successfully uploads a CV", async () => {
            const mockResponse = { user_message: "pdf", ai_message: "parsed", jobs: [] };
            vi.mocked(fetch).mockResolvedValueOnce({
                ok: true,
                json: async () => mockResponse,
            } as Response);
        ...
        it("throws an error on failure", async () => {
            ...
            await expect(uploadCVRequest(mockFile)).rejects.toThrow("HTTP error! status: 413");
        });
    });
});
